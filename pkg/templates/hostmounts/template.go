	...
	templates.Register(check.Template{
		...
		Instantiate: params.WrapInstantiateFunc(func(p params.Params) (check.Func, error) {
			return func(_ lintcontext.LintContext, object lintcontext.Object) []diagnostic.Diagnostic {
				...
				regSock, err := regexp.Compile("docker.sock$")
				if err != nil {
					return nil
				}
				...
				for _, v := range podSpec.Volumes {
					...
					for _, dir := range p.Dirs {
						fmt.Printf("hostpath = %s, forbidden_dir = %s, match=%v\n", v.HostPath.Path, dir, regSock.MatchString(v.HostPath.Path))
						if !regSock.MatchString(dir) && v.HostPath.Path != dir {
							continue
						}
						if regSock.MatchString(dir) && !regSock.MatchString(v.HostPath.Path) {
							continue
						}
						...
						for _, container := range containers {
							for _, mount := range container.VolumeMounts {
								if mount.Name == v.Name {
									results = append(results, diagnostic.Diagnostic{
										Message: fmt.Sprintf("host system directory %q is mounted on container %q", dir, container.Name)})
								}
							}
						}
					}
